...
def writeBug(bug_src,opts,bugReport):
    ctools.execmd("cp "+bug_src+" "+bugReport+"/a.c")
    ctools.execmd("touch "+" "+bugReport+"/bug_report.txt")
    ftools.put_file_content(bugReport+"/bug_report.txt",opts)
    ftools.put_file_content(bugReport+"/bug_report.txt","\n")
                    ...
                    if bugType != 'success':
                        ...
                        if "timeout" not in bugType:
                            ...
                            if mapTuple.__contains__(','.join(strNums)):
                                ...
                                bug_id=str(self.idx)+str(idxx)+str(random.randint(1,10000000))
                                bugReport = os.getcwd()+"/bugs/"+bug_id+"/"
                                ctools.execmd("mkdir "+bugReport)
                                writeBug(program,' '.join(bugTriggers),bugReport)
                                ...
                                # ftools.put_file_content(bugRecord, '\n')
                                # ftools.put_file_content(bugRecord, str(needTestPlans))
                                # ftools.put_file_content(bugRecord, '\n')
                                ftools.put_file_content(bugRecord, bugReport)
                                ftools.put_file_content(bugRecord, '\n')
